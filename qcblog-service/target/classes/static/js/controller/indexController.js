app.controller('indexController', function ($scope, $http, adminService, noticeService) {
    //显示当前用户名
    ...
    //标记公告已读状态
    $scope.updateNoticeStatus = function (id) {
        noticeService.updateNoticeStatus(id).success(
            function (response) {
                window.location.reload();
            }
        )
        /* var request = new XMLHttpRequest();
        request.open("GET", '/Notice/updateStatus?id='+id, true);
        request.send();
        request.onreadystatechange = function () {
            if (request.readyState === 4 && request.status === 200) {
                document.getElementById('slide').innerHTML = request.responseText;
            }
        }*/
    }
    //统计未读消息
    $scope.countByStatus = function () {
        noticeService.countByStatus().success(
            function (response) {
                $scope.countByStatus = response.countByStatus;
            }
        )
    }
});
